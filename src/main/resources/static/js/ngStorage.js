   ...
   if (typeof define === 'function' && define.amd) { 
    ...
    // but misses .module we can fall back to using window.
     ...
     function isStorageSupported($window, storageType) { 
       ...
       // Some installations of IE, for an unknown reason, throw "SCRIPT5:
		// Error: Access is denied"
       // when accessing window.localStorage. This happens before you try to do
		// anything with it. Catch
       // that error and allow execution to continue.
 

       // fix 'SecurityError: DOM Exception 18' exception in Desktop Safari,
		// Mobile Safari
      ...
      var supported; 
       ...
       // When Safari (OS X or iOS) is in private browsing mode, it appears as
		// though localStorage and sessionStorage
         ...
         return function () { 
          ...
          this.$get = [ 
              ...
              function( 
                 ...
                 // The magic number 10 is used which only works for some
					// keyPrefixes...
                 // See https://github.com/gsklee/ngStorage/issues/137
                 var prefixLength = storageKeyPrefix.length; 
 
                 // #9: Assign a placeholder object if Web Storage is
					// unavailable to prevent breaking the entire AngularJS app
                 // Note: recheck mainly for testing (so we can use
					// $window[storageType] rather than window[storageType])
                     ...
                     $storage = { 
                         ...
                         $sync: function () { 
                             for (var i = 0, l = webStorage.length, k; i < l; i++) { 
                                 // #8, #10: `webStorage.key(i)` may be an
									// empty string (or throw an exception in
									// IE9 if `webStorage` is empty)
                ...
                $storage.$sync(); 
                 ...
                 // #6: Use `$window.addEventListener` instead of
					// `angular.element` to avoid the jQuery-specific
					// `event.originalEvent`
